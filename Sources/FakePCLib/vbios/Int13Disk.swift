...
// Setup BIOS disk numbers and boot from a drive according to the boot order.
extension Disk {
    ...
    static private var disks: [UInt8: PhysicalMedia] = [:]

    static internal func setupDisks(_ isa: ISA) {
...
extension Disk {
    ...
    static internal func diskCall(function: UInt8, vm: VirtualMachine, isa: ISA) {
        ...
        logger.debug("DISK: function: \(diskFunction), drive: \(hexNum(drive))")
